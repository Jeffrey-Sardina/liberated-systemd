...
Liberated `systemd` will be updated at least weekly. Note that the base `systemd` repo is updated very frequently -- typiccally 2-30 commits in a day. Many of these come from merging PRs with multiple commits in their history. As a result, it's quite common to see Liberated `system` behind by 50 or more commits -- even when it's code is only a few days behind. So check on the commit dates as well if you want to know how up-to-date Liberated `systemd` is.

I do currently have a setup that would allow automating these udpates. I have so far hesitated from implemeting that, since I apprecaite scanning the new commits mantually to make sure there are no more surveillance-enabling changes. So ar this approach has worked -- and I'll keep to this update process for now. If the manner in which I maintain this changes, I'll update that here.
